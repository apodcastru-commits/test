...
class WC_Booking_Form {
		...
		if ( 'customer' === $this->product->get_duration_type() ) {
			...
			switch ( $this->product->get_duration_unit() ) {
				...
				case 'day':
					...
					} elseif ( 1 === ( $this->product->get_duration() / 7 ) ) {
							$after = __( 'Week(s)', 'woocommerce-bookings' );
					} else {
						/* translators: %s: product duration in weeks */
    ...
    // Не сохраняем в кэш, если ключ уже существует там.
    ...
    // Проверяем, совпадает ли выбранная дата с текущей датой
    $now = current_time('timestamp');
    $today = date('Ymd', $now);
    $selected_date = date('Ymd', $from);

    if ($selected_date == $today) {
        // Если даты совпадают, показываем только те блоки, которые начинаются через два часа или позже
        foreach ($available_blocks as $block => $quantity) {
            if ($quantity['available'] > 0 && $block >= $now + 2 * HOUR_IN_SECONDS) {
                $data = $this->get_end_times($blocks, get_time_as_iso8601($block), $intervals, $resource_id, $from, $to, true);
                if (!empty($data)) {
                    $block_html .= '<option data-block="' . esc_attr(date('Hi', $block)) . '" value="' . esc_attr(get_time_as_iso8601($block)) . '">' . date_i18n(wc_bookings_time_format(), $block) . '</option>';
                }
            }
        }
    } else {
        // Если даты не совпадают, показываем все доступные блоки
        foreach ($available_blocks as $block => $quantity) {
		...
		foreach ( $data as $booking_data ) {
			...
			if ( $this->product->has_resources() ) {
				...
				if ( 1 === count( $resource_ids ) ) {
					...
					$resource_qty = $this->product->get_available_quantity( $resource_ids[0] );
