...
class WC_Global_Availability_Data_Store extends WC_Data_Store_WP {
	...
	public function read( &$availability ) {
		...
		if ( false === $data ) {
			$data = $wpdb->get_row(
				$wpdb->prepare(
					'SELECT
								...
								date_modified,
       							rrule
							FROM ' . $wpdb->prefix . self::TABLE_NAME . // phpcs:ignore WordPress.DB.PreparedSQL
							' WHERE ID = %d LIMIT 1;',
					$availability->get_id()
				),
				ARRAY_A
			...
			if ( empty( $data ) ) {
				throw new Exception( esc_html__( 'Invalid event.', 'woocommerce-bookings' ) );
			}
	...
	public function has_google_event() {
		...
		return (bool) $wpdb->get_var(
			"SELECT ID FROM {$table_name} WHERE gcal_event_id != '' LIMIT 1" // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- old code, false positive.
		);
	}
